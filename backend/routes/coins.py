...
from game_engine import calculate_coins_for_action, calculate_user_tier, check_daily_earning_cap
...
async def earn_coins(request: EarnRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        today_transactions = [
            h for h in history
            if h["created_at"].startswith("2024")  # Simple date filter for demo
        ]
        ...
        new_tier = calculate_user_tier(user["coin_balance"], verified_count)

        # Update tier if changed
        if new_tier != user["tier"]:
            db.update_user_tier(request.user_id, new_tier)
